...
- Stable checkpoint: `fb2e31c fix: keep pause overlay above battle ui`
- This is the current rollback/reference point for the Godot prototype after the v04 runtime orb visual pass, chain/pacing rule pass, hazard lifecycle tuning, orb visual rotation pass, fast-drop spawn-lane stability pass, GDScript reload-warning cleanup, v05 layout art pass, grouped battle UI roots, stacked Boss HP / Boss Action UI pass, v06 single preview queue pass, combat/tactical orb cleanup, start scene pass, shield badge pass, and pause/navigation UI pass: color-chain combat effects, flashing chains that absorb late same-color members with tunable timer extension, configurable color-orb generation, tuned warning/danger hazard behavior, contact-only rolling visual offsets, shared-queue hazard/fast-drop behavior, player fast-drop spawn-lane spacing that treats attached orbs as blockers, themed board/background rendering, v04 runtime orb body/glow sprites aligned to gameplay radius, clean headless reload output, editor-adjustable UI roots, clipped stacked Boss Action fill/warning/glow behavior, the v09 stacked Boss HP / Boss Action shell with a separately clipped dynamic HP fill visible before gameplay starts, a start screen that enters the main battle scene, a HP-adjacent shield badge that displays stored shield/blue mitigation and hides at zero, and an Esc pause overlay with Main Menu / Restart / End Game buttons drawn above all battle UI.

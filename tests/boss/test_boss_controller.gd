...
func test_action_bar_volley_default_first_attack_spawns_one_hazard(runner: TestRunner) -> void:
	var battle := BattleState.new()
	var boss := BossController.new()
	var volley := ActionBarVolleyMechanic.new()
	boss.configure([volley])
	var events := boss.tick(10.0, battle)
	runner.assert_eq(events.size(), 1, "default first volley emits one event")
	runner.assert_eq(events[0].count, 1, "default first volley starts with one hazard")
	boss.free()

...
func test_v05_layout_uses_safe_margins_and_quiet_queue_chrome(runner: TestRunner) -> void:
	...
	runner.assert_true(queue_frame.texture.resource_path.ends_with("vertical_preview_queue_frame_v06.png"), "left queue uses the v06 single continuous preview frame")
	runner.assert_true(queue_frame.size.x > 145.0 and queue_frame.size.x < 155.0, "v06 queue frame is scaled wide enough for the current orb icons")
	runner.assert_true(queue_frame.size.y > 310.0 and queue_frame.size.y < 325.0, "v06 queue frame keeps the native 360x760 aspect ratio")
	...
	runner.assert_true(menu_overlay.get_index() > preview_row.get_parent().get_index(), "battle menu overlay draws above the preview queue")
	runner.assert_true(menu_overlay.get_index() > player_hp.get_index(), "battle menu overlay draws above player HP")
	runner.assert_true(menu_overlay.z_index > player_hp.z_index, "battle menu overlay has an explicit top z-index")
	...
	runner.assert_eq(preview_row.anchor_left, 0.5, "preview icons stay anchored to the v06 socket column")
	runner.assert_true(preview_row.offset_left > -20.0 and preview_row.offset_left < -16.0, "preview icons are centered on the v06 socket column")
	runner.assert_true(preview_row.offset_top > 29.0 and preview_row.offset_top < 34.0, "preview icons start on the first v06 socket")
	runner.assert_true(preview_row.get_theme_constant("separation") <= 3, "preview icon spacing follows the v06 socket spacing")
